...
pub async fn start_metrics_server(
    ...
    tracing::info!("Metrics server listening on http://0.0.0.0:{}/metrics", config.port);
...
async fn render_metrics(state: &SharedMetricsState) -> String {
    ...
    output.push_str("# HELP rivven_connect_source_running Whether source is running (1=yes, 0=no)\n");
    output.push_str("# TYPE rivven_connect_source_running gauge\n");
    for (name, metrics) in &state.sources {
        let running = if metrics.is_running.load(Ordering::Relaxed) { 1 } else { 0 };
        output.push_str(&format!(
    ...
    output.push_str("# HELP rivven_connect_sink_rate_limited_events_total Events throttled by rate limiter\n");
    ...
    output.push_str("# HELP rivven_connect_broker_connection_attempts_total Total broker connection attempts\n");
    ...
    output.push_str("# HELP rivven_connect_broker_connection_failures_total Total broker connection failures\n");
    ...
    async fn test_render_metrics() {
        let state = Arc::new(RwLock::new(MetricsState {
            ...
            sources: {
                ...
                map.insert("test_source".to_string(), ConnectorMetrics {
            ...
            sinks: {
                let mut map = std::collections::HashMap::new();
                map.insert("test_sink".to_string(), ConnectorMetrics {
        ...
        assert!(output.contains("rivven_connect_sink_rate_limited_events_total{sink=\"test_sink\"} 50"));
        assert!(output.contains("rivven_connect_sink_rate_limit_wait_ms_total{sink=\"test_sink\"} 1234"));
    }
}
