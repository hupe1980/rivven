...
impl State {
    ...
    pub fn set_global_from<T: Serialize>(
        &mut self,
...
impl StateBuilder {
    ...
    pub fn stream_cursor(
        ...
        self.state
            .update_cursor(stream_name, cursor_field, cursor_value);
        self
    }
...
impl StreamState {
    ...
    pub fn cursor(mut self, field: impl Into<String>, value: serde_json::Value) -> Self {
...
mod tests {
    ...
    fn test_state_typed_access() {
        ...
        assert_eq!(
            state.get_global_as::<String>("name"),
            Some("test".to_string())
        );
    ...
    fn test_stream_state_cursor_typed() {
        let stream = StreamState::new("test").cursor("id", json!(12345));

        assert!(stream.has_cursor());
        assert_eq!(stream.cursor_as_i64(), Some(12345));

        let stream2 = StreamState::new("test2").cursor("timestamp", json!("2026-01-25"));
