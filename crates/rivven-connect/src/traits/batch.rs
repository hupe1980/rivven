...
impl Batcher {
    ...
    pub fn add(&mut self, event: SourceEvent) -> Option<Batch> {
        ...
        let would_exceed_bytes = self.config.max_bytes > 0 
            && self.current_bytes + event_size > self.config.max_bytes;
...
impl Batch {
    ...
    pub fn from_events(events: Vec<SourceEvent>) -> Self {
        let total_bytes: usize = events.iter()
...
mod tests {
    ...
    fn test_batcher_bytes_limit() {
        let mut batcher = Batcher::new()
            .with_max_size(1000)
            .with_max_bytes(100); // Very small byte limit
    ...
    fn test_partition_events() {
        ...
        let partitions = partition_events(events, |e| {
            e.stream.clone()
        });
