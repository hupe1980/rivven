...
mod tests {
    ...
    fn test_remove_fields() {
        let mut event = SourceEvent::record("test", json!({
            "id": 1,
            "name": "test",
            "secret": "password",
            "internal": true
    ...
    fn test_map_field() {
        ...
        transforms::map_field(&mut event, "count", |v| {
    ...
    fn test_field_equals() {
        ...
        assert!(!transforms::field_equals(&event, "status", &json!("inactive")));
    }
    ...
    fn test_copy_field() {
        ...
        assert_eq!(transforms::get_field(&event, "original_id"), Some(&json!(123)));
    }
