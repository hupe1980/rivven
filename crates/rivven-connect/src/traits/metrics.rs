...
impl Timer {
    ...
    pub fn stop(mut self) -> Duration {
        ...
        if self.labels.is_empty() {
            ...
            self.collector.histogram_with_labels(&self.name, duration.as_secs_f64(), &self.labels);
        }
        duration
    }
...
impl MetricsCollector {
    ...
    fn key_with_labels(name: &str, labels: &[Label]) -> String {
        if labels.is_empty() {
            ...
            let label_str: Vec<_> = labels.iter()
...
impl Metrics for MetricsCollector {
    ...
    fn snapshot(&self) -> MetricsSnapshot {
        ...
        for (name, counter) in counters.iter() {
            metrics.insert(name.clone(), MetricValue::Counter(counter.load(Ordering::Relaxed)));
        }
        ...
        for (name, values) in histograms.iter() {
            ...
            metrics.insert(name.clone(), MetricValue::Histogram(HistogramSnapshot {
                count,
                sum,
                min,
                max,
...
mod tests {
    ...
    fn test_metrics_with_labels() {
        ...
        let snapshot = collector.snapshot();
        assert_eq!(snapshot.counter("records{source=postgres,table=users}"), Some(10));
    }
