...
impl BrokerClient {
    ...
    pub async fn publish_with_key(
        ...
        let client = guard.as_mut().ok_or_else(|| {
            ConnectError::broker("Not connected to broker")
        })?;
                ...
                if Self::is_connection_error(&e) {
                    ...
                    *self.status.write().await = ConnectorStatus::Unhealthy;
                    Err(ConnectError::broker(format!(
                        "Connection error during publish: {}", e
                    )))
                } else {
...
impl BrokerClient {
    ...
    async fn try_connect_with_failover(&self) -> std::result::Result<(Client, String), rivven_client::Error> {
        ...
        for server in &self.config.bootstrap_servers {
            ...
            match tokio::time::timeout(timeout, self.try_connect_to(server)).await {
                ...
                Err(_) => {
                    ...
                    last_error = Some(rivven_client::Error::ConnectionError(
    ...
    async fn try_connect_to(&self, address: &str) -> std::result::Result<Client, rivven_client::Error> {
        #[cfg(feature = "tls")]
    ...
    #[cfg(feature = "tls")]
    async fn connect_tls_to(&self, address: &str) -> std::result::Result<Client, rivven_client::Error> {
        ...
        let tls_config = match (&self.config.tls.cert_path, &self.config.tls.key_path) {
            ...
            _ => {
                ...
                if let Some(ca) = &self.config.tls.ca_path {
                    tls.root_ca = Some(rivven_core::tls::CertificateSource::File {
                        path: ca.clone(),
                    });
                }
        ...
        let server_name = self.config.tls.server_name.as_deref().unwrap_or("localhost");
    ...
    pub async fn create_topic(&self, name: &str, partitions: u32) -> Result<()> {
        ...
        client.create_topic(name, Some(partitions)).await.map_err(|e| {
    ...
    pub async fn topic_exists(&self, name: &str) -> Result<bool> {
        ...
        let topics = client.list_topics().await.map_err(|e| {
            ConnectError::broker(format!("Failed to list topics: {}", e))
        })?;
    ...
    pub async fn list_topics(&self) -> Result<Vec<String>> {
        ...
        client.list_topics().await.map_err(|e| {
            ConnectError::broker(format!("Failed to list topics: {}", e))
        })
    }
        ...
        match client.consume(topic, partition, offset, max_messages).await {
            ...
            Err(e) => {
                if Self::is_connection_error(&e) {
                    ...
                    *self.status.write().await = ConnectorStatus::Unhealthy;
                    Err(ConnectError::broker(format!(
                        "Connection error during consume: {}", e
                    )))
                } else {
    ...
    pub async fn commit_offset(
        ...
        client.commit_offset(consumer_group, topic, partition, offset)
            .await
    ...
    pub async fn get_offset_bounds(
        ...
        client.get_offset_bounds(topic, partition)
            .await
    ...
    pub async fn get_offset_for_timestamp(
        ...
        client.get_offset_for_timestamp(topic, partition, timestamp_ms)
            .await
