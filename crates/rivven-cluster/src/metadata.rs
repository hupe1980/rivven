...
pub enum MetadataResponse {
    ...
    PartitionLeaderUpdated { partition: PartitionId, leader: NodeId },
    IsrUpdated { partition: PartitionId, isr: Vec<NodeId> },
    NodeRegistered { node_id: NodeId },
    NodeDeregistered { node_id: NodeId },
    ConsumerGroupUpdated { group_id: GroupId },
    ConsumerGroupDeleted { group_id: GroupId },
...
impl ClusterMetadata {
    ...
    pub fn apply(&mut self, index: u64, cmd: MetadataCommand) -> MetadataResponse {
        ...
        match cmd {
            MetadataCommand::CreateTopic { config, partition_assignments } => {
            ...
            MetadataCommand::AddPartitions { topic, new_assignments } => {
                self.add_partitions(&topic, new_assignments)
            }
            MetadataCommand::UpdatePartitionLeader { partition, leader, epoch } => {
            ...
            MetadataCommand::ReassignPartition { partition, replicas } => {
            ...
            MetadataCommand::UpdateNode { node_id, info } => {
                self.update_node(&node_id, info)
            }
            MetadataCommand::UpdateClusterConfig { config } => {
                self.update_cluster_config(config)
            }
            MetadataCommand::UpdateConsumerGroup { group } => {
                self.update_consumer_group(group)
            }
    ...
    /// Create a new topic
    fn create_topic(&mut self, config: TopicConfig, partition_assignments: Vec<Vec<NodeId>>) -> MetadataResponse {
    ...
    fn delete_topic(&mut self, name: &str) -> MetadataResponse {
        if self.topics.remove(name).is_some() {
            self.epoch += 1;
            MetadataResponse::TopicDeleted { name: name.to_string() }
        } else {
    ...
    /// Update topic configuration
    fn update_topic_config(&mut self, name: &str, config: HashMap<String, String>) -> MetadataResponse {
    ...
    fn add_partitions(&mut self, topic: &str, new_assignments: Vec<Vec<NodeId>>) -> MetadataResponse {
    ...
    /// Update partition leader
    fn update_partition_leader(&mut self, partition: &PartitionId, leader: NodeId, epoch: u64) -> MetadataResponse {
    ...
    /// Update partition ISR
    fn update_partition_isr(&mut self, partition: &PartitionId, isr: Vec<NodeId>) -> MetadataResponse {
        if let Some(topic) = self.topics.get_mut(&partition.topic) {
            if let Some(p) = topic.partition_mut(partition.partition) {
    ...
    /// Reassign partition replicas
    fn reassign_partition(&mut self, partition: &PartitionId, replicas: Vec<NodeId>) -> MetadataResponse {
        if let Some(topic) = self.topics.get_mut(&partition.topic) {
            if let Some(p) = topic.partition_mut(partition.partition) {
                // Create new replica info
                p.replicas = replicas.iter()
    ...
    fn deregister_node(&mut self, node_id: &NodeId) -> MetadataResponse {
        if self.nodes.remove(node_id).is_some() {
            self.epoch += 1;
            MetadataResponse::NodeDeregistered { node_id: node_id.clone() }
        } else {
            MetadataResponse::Error {
    ...
    fn delete_consumer_group(&mut self, group_id: &GroupId) -> MetadataResponse {
        if self.consumer_groups.remove(group_id).is_some() {
            MetadataResponse::ConsumerGroupDeleted { group_id: group_id.clone() }
        } else {
            MetadataResponse::Error {
...
mod tests {
    ...
    fn test_create_topic() {
        ...
        let cmd = MetadataCommand::CreateTopic { config, partition_assignments: assignments };
    ...
    fn test_update_partition_leader() {
        ...
        metadata.apply(1, MetadataCommand::CreateTopic { config, partition_assignments: assignments });
        ...
        let result = metadata.apply(2, MetadataCommand::UpdatePartitionLeader {
            partition: partition.clone(),
            leader: "node-2".to_string(),
            epoch: 2,  // Must be higher than current epoch (1)
        });
        
        assert!(matches!(result, MetadataResponse::PartitionLeaderUpdated { .. }));
        assert_eq!(
    ...
    fn test_register_deregister_node() {
        ...
        // Deregister
        let result = metadata.apply(2, MetadataCommand::DeregisterNode { node_id: "node-1".to_string() });
    ...
    fn test_serialization() {
        let mut metadata = ClusterMetadata::new();
        metadata.apply(1, MetadataCommand::CreateTopic {
