...
use crate::protocol::{
    ...
    frame_length, frame_message, ClusterRequest, ClusterResponse,
};
use dashmap::DashMap;
...
impl Transport {
    ...
    async fn handle_connection(
        ...
        loop {
            ...
            timeout(config.read_timeout, stream.read_exact(&mut body)).await
            ...
            if let Ok(request) = decode_request(&body) {
                ...
                if let Some(ref handler) = handler {
                    ...
                    timeout(config.write_timeout, stream.write_all(&framed)).await
                        .map_err(|_| ClusterError::Timeout)?
                        .map_err(ClusterError::Io)?;
                }
    ...
    pub async fn send(&self, node_id: &NodeId, request: ClusterRequest) -> Result<ClusterResponse> {
        ...
        timeout(self.config.write_timeout, stream.write_all(&framed)).await
            .map_err(|_| ClusterError::Timeout)?
            .map_err(ClusterError::Io)?;
        ...
        // Read response
        let mut length_buf = [0u8; 4];
        timeout(self.config.read_timeout, stream.read_exact(&mut length_buf)).await
            .map_err(|_| ClusterError::Timeout)?
            .map_err(ClusterError::Io)?;
        
        let length = frame_length(&length_buf);
        let mut body = vec![0u8; length];
        timeout(self.config.read_timeout, stream.read_exact(&mut body)).await
            .map_err(|_| ClusterError::Timeout)?
            .map_err(ClusterError::Io)?;
