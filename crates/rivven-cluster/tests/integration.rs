...
use rivven_cluster::{
    ClusterConfig, ClusterCoordinator, ClusterMode, TopicConfig,
    SwimConfig, RaftConfig, ReplicationConfig, TopicDefaults,
};
...
async fn test_health_reporting() {
    ...
    coordinator.create_topic(TopicConfig::new("health-test-1", 2, 1)).await.unwrap();
    coordinator.create_topic(TopicConfig::new("health-test-2", 3, 1)).await.unwrap();
...
async fn test_metadata_replication_simulation() {
    ...
    coord1.create_topic(TopicConfig::new("topic-1", 2, 1)).await.unwrap();
    coord2.create_topic(TopicConfig::new("topic-2", 3, 1)).await.unwrap();
...
async fn test_replication_factor_validation() {
    ...
    let result = coordinator.create_topic(TopicConfig::new("invalid", 2, 3)).await;
...
async fn test_duplicate_topic_creation() {
    ...
    // Create a topic
    coordinator.create_topic(TopicConfig::new("duplicate", 2, 1)).await.unwrap();
    
    // Creating again should fail
    let result = coordinator.create_topic(TopicConfig::new("duplicate", 2, 1)).await;
    assert!(result.is_err());
}
