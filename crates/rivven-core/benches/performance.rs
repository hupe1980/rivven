...
fn bench_single_vs_batch(c: &mut Criterion) {
    ...
    group.bench_function("batch_append_1000", |b| {
        let config = get_test_config();
        let partition = rt.block_on(async {
            Arc::new(Partition::new(&config, "bench-batch", 0).await.unwrap())
        });
...
fn bench_schema_caching(c: &mut Criterion) {
    ...
    use rivven_core::schema_registry::{SchemaRegistry, MemorySchemaRegistry};
    ...
    let schema_id = rt.block_on(async {
        registry.register("user-value", schema_str).await.unwrap()
    });
...
fn bench_concurrent_appends(c: &mut Criterion) {
    ...
    for num_threads in [1, 2, 4, 8, 16].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(num_threads), num_threads, |b, &threads| {
            let config = get_test_config();
            let partition = rt.block_on(async {
                Arc::new(Partition::new(&config, "bench-concurrent", 0).await.unwrap())
            });
