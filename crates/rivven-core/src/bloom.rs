...
impl CountingBloomFilter {
    ...
    pub fn new(expected_items: usize, false_positive_rate: f64) -> Self {
        let ln2_squared = std::f64::consts::LN_2 * std::f64::consts::LN_2;
        let num_counters = (-(expected_items as f64) * false_positive_rate.ln() / ln2_squared).ceil() as usize;
        let num_counters = num_counters.max(64);
        
        let num_hashes = ((num_counters as f64 / expected_items as f64) * std::f64::consts::LN_2).ceil() as usize;
...
impl OffsetBloomFilter {
    ...
    pub fn contains_range(&self, start: u64, end: u64) -> bool {
        ...
        for bucket in start_bucket..=end_bucket {
            ...
            for &offset in &[bucket_start, bucket_start + self.bucket_size / 2, bucket_end] {
...
mod tests {
    ...
    fn test_hyperloglog() {
        ...
        assert!(error < 0.1, "Estimate {} too far from 10000 (error: {}%)", estimate, error * 100.0);
    }
    ...
    fn test_hyperloglog_merge() {
        ...
        let error = (estimate as i64 - 10000i64).abs() as f64 / 10000.0;
        assert!(error < 0.15, "Merged estimate {} too far from 10000", estimate);
    }
