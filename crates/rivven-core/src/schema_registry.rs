...
impl EmbeddedSchemaRegistry {
    ...
    async fn recover(&self) -> Result<()> {
                        ...
                        if let Ok(event) = bincode::deserialize::<SchemaLogEvent>(&msg.value) {
                            match event {
                                SchemaLogEvent::Register(reg) => {
                                    self.cache.inject_registration(reg.id, &reg.subject, &reg.schema, reg.version)?;
                                }
                            }
                        }
                    }
        ...
        let max_id = self.cache.id_to_schema.iter()
            .map(|entry| *entry.key())
            .max()
            .unwrap_or(0);
...
impl SchemaRegistry for EmbeddedSchemaRegistry {
    async fn register(&self, subject: &str, schema_str: &str) -> Result<i32> {
        ...
        let version = self.cache.subject_versions.get(subject)
            .map(|v| {
                v.iter()
                    .map(|entry| *entry.key())
                    .max()
        ...
        self.cache.inject_registration(next_id, subject, schema_str, version)?;
...
impl MemorySchemaRegistry {
    ...
    pub fn inject_registration(&self, id: i32, subject: &str, schema_str: &str, version: i32) -> Result<()> {
        ...
        let versions = self.subject_versions.entry(subject.to_string())
            .or_default();
    ...
    pub async fn check_existing(&self, subject: &str, schema_str: &str) -> std::result::Result<i32, ()> {
...
impl SchemaRegistry for MemorySchemaRegistry {
    async fn register(&self, subject: &str, schema_str: &str) -> Result<i32> {
        ...
        let version = self.subject_versions.get(subject)
            .map(|v| {
                v.iter()
                    .map(|entry| *entry.key())
                    .max()
...
mod tests {
    ...
    async fn test_embedded_registry_persistence() {
        ...
        {
            let registry = EmbeddedSchemaRegistry::new(&config).await.expect("Failed to create registry");
            let id = registry.register(subject, schema_str).await.expect("Failed to register");
            assert_eq!(id, 1);
        ...
        {
            let registry = EmbeddedSchemaRegistry::new(&config).await.expect("Failed to recover registry");
            
            // Should have id 1
            let schema = registry.get_schema(1).await.expect("Failed to get schema after recovery");
            assert!(format!("{:?}", schema).contains("User"));

            // Check caching of subject version
            let latest = registry.get_latest_schema_id(subject).await.expect("Failed to get latest").unwrap();
            assert_eq!(latest, 1);

            // 3. Register next schema
            let schema_str2 = r#"{"type":"record","name":"UserV2","fields":[{"name":"age","type":"int"}]}"#;
            let id2 = registry.register(subject, schema_str2).await.expect("Failed to register 2");
            assert_eq!(id2, 2);
        }
