...
impl HotTier {
    ...
    pub async fn remove(&self, topic: &str, partition: u32, base_offset: u64) -> Option<Bytes> {
        ...
        if let Some(ref data) = removed {
            self.current_size.fetch_sub(data.len() as u64, Ordering::Relaxed);
            let mut lru = self.lru_order.lock().await;
            lru.retain(|k| k != &key);
        }
...
impl WarmTier {
    ...
    pub async fn read(&self, topic: &str, partition: u32, base_offset: u64) -> Result<Option<Bytes>> {
        let path = self.segment_path(topic, partition, base_offset);
    ...
    pub async fn get_metadata(&self, topic: &str, partition: u32, base_offset: u64) -> Option<Arc<SegmentMetadata>> {
        let key = (topic.to_string(), partition, base_offset);
    ...
    fn key_to_path(&self, key: &str) -> Result<PathBuf> {
        ...
        if key.contains('\0') {
            return Err(Error::Other(
                "Invalid key: null byte not allowed".into(),
            ));
        }
        
        let path = self.base_path.join(key.replace('/', std::path::MAIN_SEPARATOR_STR));
...
impl ColdStorageBackend for LocalFsColdStorage {
    ...
    async fn list(&self, prefix: &str) -> Result<Vec<String>> {
        ...
        fn walk_dir(dir: &std::path::Path, base: &std::path::Path, keys: &mut Vec<String>) -> std::io::Result<()> {
            if dir.is_dir() {
...
impl TieredStorage {
    ...
    pub async fn new(config: TieredStorageConfig) -> Result<Arc<Self>> {
        ...
        let cold_storage: Arc<dyn ColdStorageBackend> = match &config.cold_storage {
            ColdStorageConfig::LocalFs { path } => {
                Arc::new(LocalFsColdStorage::new(path.clone())?)
            }
            ...
            ColdStorageConfig::AzureBlob { .. } => {
                ...
                return Err(Error::Other("Azure Blob cold storage not yet implemented".into()));
            }
        };
    ...
    pub async fn write(
        ...
        let inserted = self.hot_tier.insert(topic, partition, base_offset, data.clone()).await;
        
        if !inserted {
            // Hot tier full and can't evict, write directly to warm
            self.warm_tier.store(topic, partition, base_offset, end_offset, &data).await?;
        ...
        let metadata = Arc::new(SegmentMetadata::new(
            ...
            end_offset,
            size,
            if inserted { StorageTier::Hot } else { StorageTier::Warm },
        ));
    ...
    pub async fn read(
                    ...
                    if let Some(data) = self.warm_tier.read(topic, partition, base_offset).await? {
                        ...
                        if self.config.enable_promotion {
                            ...
                            if access_count >= self.config.promotion_threshold {
                                let _ = self.migration_tx.send(MigrationTask::Promote {
        ...
        self.stats.total_bytes_read.fetch_add(bytes_collected as u64, Ordering::Relaxed);
    ...
    pub async fn flush_hot_tier(&self, topic: &str, partition: u32) -> Result<()> {
        ...
        for base_offset in segments {
            let _ = self.migration_tx.send(MigrationTask::Demote {
                topic: topic.to_string(),
                partition,
                base_offset,
    ...
    async fn check_tier_migrations(&self) -> Result<()> {
        ...
        for (topic, partition, base_offset) in compaction_candidates {
            let _ = self.migration_tx.send(MigrationTask::Compact {
                topic,
                partition,
                base_offset,
            }).await;
    ...
    async fn execute_migration(&self, task: MigrationTask) -> Result<()> {
        match task {
            MigrationTask::Demote { topic, partition, base_offset, from_tier } => {
                self.demote_segment(&topic, partition, base_offset, from_tier).await
            }
            MigrationTask::Promote { topic, partition, base_offset, to_tier } => {
                self.promote_segment(&topic, partition, base_offset, to_tier).await
            }
            MigrationTask::Compact { topic, partition, base_offset } => {
    ...
    async fn demote_segment(
        ...
        let data = match from_tier {
            ...
            StorageTier::Warm => {
                self.warm_tier.read(topic, partition, base_offset).await?
            }
            StorageTier::Cold => None,
        };
        ...
        let metadata = self.get_segment_metadata(topic, partition, base_offset).await;
        let end_offset = metadata.as_ref().map(|m| m.end_offset).unwrap_or(base_offset);
        ...
        tracing::debug!(
            ...
            topic, partition, base_offset, from_tier, to_tier
        );
    ...
    async fn promote_segment(
        ...
        let metadata = match self.get_segment_metadata(topic, partition, base_offset).await {
            Some(m) => m,
            None => return Ok(()),
        };
        ...
        match to_tier {
            StorageTier::Hot => {
                self.hot_tier.insert(topic, partition, base_offset, data).await;
            }
            StorageTier::Warm => {
                self.warm_tier.store(topic, partition, base_offset, metadata.end_offset, &data).await?;
            }
    ...
    async fn compact_segment(
        ...
        let metadata = match self.get_segment_metadata(topic, partition, base_offset).await {
            Some(m) => m,
            None => {
                tracing::debug!("Segment not found for compaction: {}/{}/{}", topic, partition, base_offset);
                return Ok(());
            }
        };
        ...
        let data = match metadata.tier {
            StorageTier::Hot => {
                self.hot_tier.get(topic, partition, base_offset).await
            }
            StorageTier::Warm => {
                self.warm_tier.read(topic, partition, base_offset).await?
            }
        ...
        let data = match data {
            Some(d) => d,
            None => {
                tracing::debug!("Segment data not found for compaction: {}/{}/{}", topic, partition, base_offset);
                return Ok(());
            }
        };
        ...
        while cursor < data.len() {
            ...
            if cursor + len > data.len() {
                tracing::warn!("Truncated message in segment {}/{}/{}", topic, partition, base_offset);
                break;
            }
        ...
        if messages.is_empty() {
            tracing::debug!("No messages to compact in segment {}/{}/{}", topic, partition, base_offset);
            return Ok(());
        }
        ...
        if compacted_count >= original_count {
            tracing::debug!(
                ...
                topic, partition, base_offset, original_count, compacted_count
            );
            return Ok(());
        }
        ...
        tracing::info!(
            ...
            original_count, compacted_count, reduction_ratio * 100.0
        );
        ...
        match metadata.tier {
            StorageTier::Hot => {
                ...
                self.hot_tier.remove(topic, partition, base_offset).await;
                self.hot_tier.insert(topic, partition, base_offset, compacted_bytes).await;
            }
            StorageTier::Warm => {
                // Remove and re-store
                self.warm_tier.remove(topic, partition, base_offset).await?;
                self.warm_tier.store(topic, partition, base_offset, new_end_offset, &compacted_bytes).await?;
            }
            StorageTier::Cold => {
...
mod tests {
    ...
    async fn test_tiered_storage_write_and_read() {
        ...
        storage.write("topic1", 0, 0, 10, data.clone()).await.unwrap();
    ...
    async fn test_segment_compaction() {
        ...
        storage.write("compaction-test", 0, 0, 4, segment_bytes).await.unwrap();
        
        // Get metadata and trigger compaction
        let meta = storage.get_segment_metadata("compaction-test", 0, 0).await.unwrap();
        
        // Simulate dead records (2 out of 4 will be removed)
        meta.dead_records.store(2, Ordering::Relaxed);
        
        // Run compaction
        storage.compact_segment("compaction-test", 0, 0).await.unwrap();
        
        // Verify compacted segment has fewer messages
        let meta_after = storage.get_segment_metadata("compaction-test", 0, 0).await.unwrap();
        assert!(meta_after.total_records < 4, "Compaction should reduce message count");
    ...
    async fn test_compaction_preserves_keyless_messages() {
        ...
        let segment_bytes = Bytes::from(segment_data);
        storage.write("keyless-test", 0, 0, 5, segment_bytes).await.unwrap();
        
        // Run compaction
        storage.compact_segment("keyless-test", 0, 0).await.unwrap();
        
        // All keyless messages should be preserved (no dedup for keyless)
        let meta_after = storage.get_segment_metadata("keyless-test", 0, 0).await.unwrap();
        assert_eq!(meta_after.total_records, 5, "Keyless messages should all be preserved");
