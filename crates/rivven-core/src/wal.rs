...
impl TryFrom<u8> for RecordType {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            _ => Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid record type")),
        }
    }
}
...
impl WalRecord {
    ...
    pub fn from_bytes(data: &[u8], lsn: u64) -> io::Result<Self> {
        ...
        if data.len() < RECORD_HEADER_SIZE + data_len {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "Incomplete record"));
        }
        
        let record_data = Bytes::copy_from_slice(&data[RECORD_HEADER_SIZE..RECORD_HEADER_SIZE + data_len]);
...
impl GroupCommitWal {
    ...
    async fn recover_state(config: &WalConfig) -> io::Result<(PathBuf, u64)> {
        ...
        let file = latest_file.unwrap_or_else(|| {
            config.dir.join(format!("{:020}.wal", 0))
        });
    ...
    pub async fn write_with_type(&self, data: Bytes, record_type: RecordType) -> io::Result<WriteResult> {
        if self.shutdown.load(Ordering::Acquire) {
            return Err(io::Error::new(io::ErrorKind::BrokenPipe, "WAL is shut down"));
        }
    ...
    async fn flush_batch(
        ...
        // Update stats
        self.stats.writes_total.fetch_add(group_size as u64, Ordering::Relaxed);
        self.stats.bytes_written.fetch_add(batch_buffer.len() as u64, Ordering::Relaxed);
...
impl WalWriter {
    ...
    fn find_actual_end(file: &File, file_len: u64) -> io::Result<u64> {
        ...
        while position + RECORD_HEADER_SIZE as u64 <= file_len {
            ...
            let data_len = u32::from_be_bytes([header[8], header[9], header[10], header[11]]) as u64;
...
impl WalReader {
    ...
    pub async fn seek_to_lsn(&mut self, target_lsn: u64) -> io::Result<()> {
        ...
        Err(io::Error::new(io::ErrorKind::NotFound, format!("LSN {} not found", target_lsn)))
    }
}
...
mod tests {
    ...
    async fn test_group_commit_batching() {
        ...
        for i in 0..20 {
            ...
            handles.push(tokio::spawn(async move {
                wal_clone.write(Bytes::from(format!("concurrent write {}", i))).await
            }));
        }
    ...
    async fn test_wal_reader() {
        ...
        for i in 0..5 {
            let result = wal.write(Bytes::from(format!("record {}", i))).await.unwrap();
        ...
        assert_eq!(records.len(), 5, "Expected 5 records, got {} (file size: {})", records.len(), file_size);
