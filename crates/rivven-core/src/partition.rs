use crate::{Message, Result, Config};
...
impl Partition {
    ...
    pub async fn append(&self, mut message: Message) -> Result<u64> {
        ...
        debug!("Appended message at offset {} to partition {}", offset, self.id);
    ...
    pub async fn append_batch(&self, messages: Vec<Message>) -> Result<Vec<u64>> {
        ...
        let start_offset = self.next_offset.fetch_add(batch_size as u64, Ordering::SeqCst);
        ...
        debug!("Batch appended {} messages to partition {} (offsets {}-{})", 
            batch_size, self.id, start_offset, start_offset + batch_size as u64 - 1);
...
mod tests {
    ...
    fn get_test_config() -> Config {
        ...
        config.data_dir = format!("/tmp/rivven-test-partition-{}", uuid::Uuid::new_v4());
    ...
    async fn test_partition_persistence() {
        ...
        // 1. Create partition and write messages
        {
            ...
            partition.append(Message::new(Bytes::from("msg1"))).await.unwrap();
            partition.append(Message::new(Bytes::from("msg2"))).await.unwrap();
        ...
        // 2. Re-open partition to test persistence and recovery
        {
            ...
            // Append new message
            partition.append(Message::new(Bytes::from("msg3"))).await.unwrap();
             let stored = partition.read(0, 10).await.unwrap();
    ...
    async fn test_find_offset_for_timestamp() {
        ...
        assert_eq!(found_offset, Some(2), "Should find offset 2 for timestamp {}", ts_msg2);
        
        // Find offset for timestamp before all messages - should return offset 0
        let very_old_ts = ts_msg2 - 10000; // 10 seconds before
        let found_offset = partition.find_offset_for_timestamp(very_old_ts).await.unwrap();
        assert_eq!(found_offset, Some(0), "Should find offset 0 for very old timestamp");
        ...
        let found_offset = partition.find_offset_for_timestamp(future_ts).await.unwrap();
        assert_eq!(found_offset, None, "Should return None for future timestamp");
