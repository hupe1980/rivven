...
impl MySqlTestContainer {
    ...
    pub async fn create_database(&self, name: &str) -> Result<()> {
        self.execute(&format!("CREATE DATABASE IF NOT EXISTS `{}`", name)).await
    }
    
    /// Create a replication user for CDC
    pub async fn create_replication_user(&self, user: &str, password: &str) -> Result<()> {
        let statements = [
            &format!("CREATE USER IF NOT EXISTS '{}'@'%' IDENTIFIED BY '{}'", user, password) as &str,
            &format!("GRANT REPLICATION SLAVE, REPLICATION CLIENT ON *.* TO '{}'@'%'", user),
...
async fn mysql_connect_check(host: &str, port: u16) -> Result<()> {
    ...
    // Try to connect
    let _client = MySqlBinlogClient::connect(
        host,
        port,
        "root",
        Some("rootpassword"),
