...
mod property_tests {
    ...
    async fn prop_insert_produces_one_event() {
        ...
        rivven.register_cdc_topic("postgres", "public", "prop_insert").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        ...
        let messages = rivven.wait_for_messages(
        ...
        events.assert().has_op_count(CdcOp::Insert, test_names.len());
    ...
    async fn prop_update_has_before_and_after() {
        ...
        ctx.pg.execute(
            "CREATE TABLE prop_update (id SERIAL PRIMARY KEY, value INTEGER NOT NULL)"
        ).await.unwrap();
        ctx.pg.execute("ALTER TABLE prop_update REPLICA IDENTITY FULL").await.unwrap();
        
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "prop_update").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        // Insert initial value
        ctx.pg.execute("INSERT INTO prop_update (value) VALUES (100)").await.unwrap();
        
        // Update with various delta values
        let deltas = [1, -50, 1000, -1000, 0];
        for delta in deltas.iter() {
            ctx.pg.execute(&format!("UPDATE prop_update SET value = value + {} WHERE id = 1", delta)).await.unwrap();
        }
        ...
        let updates: Vec<_> = events.iter().filter(|e| matches!(e.op, CdcOp::Update)).collect();
    ...
    async fn prop_delete_captures_deleted_data() {
        ...
        ctx.pg.execute(
            "CREATE TABLE prop_delete (id SERIAL PRIMARY KEY, name TEXT NOT NULL)"
        ).await.unwrap();
        ctx.pg.execute("ALTER TABLE prop_delete REPLICA IDENTITY FULL").await.unwrap();
        
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "prop_delete").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        // Insert then delete multiple rows
        let names = ["Alice", "Bob", "Charlie"];
        for name in names.iter() {
            ctx.pg.execute(&format!("INSERT INTO prop_delete (name) VALUES ('{}')", name)).await.unwrap();
        }
        ...
        let deletes: Vec<_> = events.iter().filter(|e| matches!(e.op, CdcOp::Delete)).collect();
        assert_eq!(deletes.len(), names.len());
        
        for delete in deletes {
            assert!(delete.before.is_some(), "Delete should capture before image");
        }
    ...
    async fn prop_transaction_atomicity() {
        ...
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "prop_tx").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        // Execute batch as single transaction
        let batch_size = 10;
        let statements: Vec<String> = (0..batch_size)
            .map(|i| format!("INSERT INTO prop_tx (name, email) VALUES ('User{}', 'user{}@test.com')", i, i))
            .collect();
    ...
    async fn prop_event_ordering_preserved() {
        ...
        ctx.pg.execute(
            "CREATE TABLE prop_order (id SERIAL PRIMARY KEY, seq INTEGER NOT NULL)"
        ).await.unwrap();
        
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "prop_order").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        // Insert with explicit sequence numbers
        let count = 20;
        for i in 0..count {
            ctx.pg.execute(&format!("INSERT INTO prop_order (seq) VALUES ({})", i)).await.unwrap();
        }
...
mod schema_evolution_tests {
    ...
    async fn test_add_column() {
        ...
        ctx.pg.execute(
            "CREATE TABLE schema_add_col (id SERIAL PRIMARY KEY, name TEXT)"
        ).await.unwrap();
        
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "schema_add_col").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        // Insert before schema change
        ctx.pg.execute("INSERT INTO schema_add_col (name) VALUES ('Before')").await.unwrap();
        
        sleep(Duration::from_millis(500)).await;
        
        // Add new column
        ctx.pg.execute("ALTER TABLE schema_add_col ADD COLUMN email TEXT").await.unwrap();
        
        // Insert after schema change
        ctx.pg.execute("INSERT INTO schema_add_col (name, email) VALUES ('After', 'after@test.com')").await.unwrap();
        ...
        assert!(second_after.get("email").is_some(), "New column should appear in CDC events");
        assert_eq!(
    ...
    async fn test_drop_column() {
        ...
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "schema_drop_col").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        ...
        ctx.pg.execute("ALTER TABLE schema_drop_col DROP COLUMN email").await.unwrap();
        
        // Insert after drop
        ctx.pg.execute("INSERT INTO schema_drop_col (name, age) VALUES ('After', 30)").await.unwrap();
        ...
        let first_after = events[0].after.as_ref().unwrap();
        assert!(first_after.get("email").is_some(), "First event should have email");
        
        // Second event should NOT have email
        let second_after = events[1].after.as_ref().unwrap();
        assert!(second_after.get("email").is_none(), "Dropped column should not appear in CDC events");
    ...
    async fn test_alter_column_type() {
        ...
        ctx.pg.execute(
            "CREATE TABLE schema_alter_type (id SERIAL PRIMARY KEY, value INTEGER)"
        ).await.unwrap();
        
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "schema_alter_type").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        // Insert integer value
        ctx.pg.execute("INSERT INTO schema_alter_type (value) VALUES (42)").await.unwrap();
        
        sleep(Duration::from_millis(500)).await;
        
        // Change column type to TEXT
        ctx.pg.execute("ALTER TABLE schema_alter_type ALTER COLUMN value TYPE TEXT").await.unwrap();
        
        // Insert text value
        ctx.pg.execute("INSERT INTO schema_alter_type (value) VALUES ('hello world')").await.unwrap();
    ...
    async fn test_rename_column() {
        ...
        ctx.pg.execute(
            "CREATE TABLE schema_rename (id SERIAL PRIMARY KEY, old_name TEXT)"
        ).await.unwrap();
        
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "schema_rename").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        // Insert with old column name
        ctx.pg.execute("INSERT INTO schema_rename (old_name) VALUES ('value1')").await.unwrap();
        
        sleep(Duration::from_millis(500)).await;
        
        // Rename column
        ctx.pg.execute("ALTER TABLE schema_rename RENAME COLUMN old_name TO new_name").await.unwrap();
        
        // Insert with new column name
        ctx.pg.execute("INSERT INTO schema_rename (new_name) VALUES ('value2')").await.unwrap();
        ...
        let first_after = events[0].after.as_ref().unwrap();
        assert!(first_after.get("old_name").is_some(), "First event should have old_name");
        
        // Second event has new_name
        let second_after = events[1].after.as_ref().unwrap();
        assert!(second_after.get("new_name").is_some(), "Second event should have new_name");
    ...
    async fn test_add_constraint() {
        ...
        ctx.pg.execute(
            "CREATE TABLE schema_constraint (id SERIAL PRIMARY KEY, email TEXT)"
        ).await.unwrap();
        
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "schema_constraint").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        // Insert before constraint
        ctx.pg.execute("INSERT INTO schema_constraint (email) VALUES ('test@example.com')").await.unwrap();
        
        sleep(Duration::from_millis(500)).await;
        
        // Add UNIQUE constraint
        ctx.pg.execute("ALTER TABLE schema_constraint ADD CONSTRAINT unique_email UNIQUE (email)").await.unwrap();
        
        // Insert after constraint (different email)
        ctx.pg.execute("INSERT INTO schema_constraint (email) VALUES ('other@example.com')").await.unwrap();
...
mod chaos_tests {
    ...
    async fn test_resilience_to_slow_operations() {
        ...
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "chaos_slow").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        // Normal insert
        ctx.pg.execute("INSERT INTO chaos_slow (name) VALUES ('before_slow')").await.unwrap();
        
        // Simulate slow operation with pg_sleep
        ctx.pg.execute("SELECT pg_sleep(0.5)").await.unwrap();
        
        // Insert after delay
        ctx.pg.execute("INSERT INTO chaos_slow (name) VALUES ('after_slow')").await.unwrap();
        
        sleep(Duration::from_millis(EVENT_PROPAGATION_DELAY_MS * 3)).await;
        
        let messages = rivven.wait_for_messages(
            "cdc.postgres.public.chaos_slow",
            2,
            Duration::from_secs(15),
        ).await.unwrap();
    ...
    async fn test_long_running_transaction() {
        ...
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "chaos_long_tx").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
    ...
    async fn test_large_values() {
        ...
        ctx.pg.execute(
            "CREATE TABLE chaos_large (id SERIAL PRIMARY KEY, data TEXT)"
        ).await.unwrap();
        
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "chaos_large").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        ...
        for (i, event) in events.iter().enumerate() {
            ...
            assert_eq!(data.len(), sizes[i], "Large value {} should be captured correctly", i);
        }
    ...
    async fn test_rapid_same_row_updates() {
        ...
        ctx.pg.execute(
            "CREATE TABLE chaos_rapid (id SERIAL PRIMARY KEY, counter INTEGER DEFAULT 0)"
        ).await.unwrap();
        ctx.pg.execute("ALTER TABLE chaos_rapid REPLICA IDENTITY FULL").await.unwrap();
        
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "chaos_rapid").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        // Insert row
        ctx.pg.execute("INSERT INTO chaos_rapid (counter) VALUES (0)").await.unwrap();
        
        // Rapid updates to same row
        let update_count = 50;
        for i in 1..=update_count {
            ctx.pg.execute(&format!("UPDATE chaos_rapid SET counter = {} WHERE id = 1", i)).await.unwrap();
        }
        ...
        let last_update = events.iter().filter(|e| matches!(e.op, CdcOp::Update)).last().unwrap();
        let counter = last_update.after.as_ref().unwrap().get("counter").unwrap().as_str().unwrap();
    ...
    async fn test_wide_table() {
        ...
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "chaos_wide").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        ...
        ctx.pg.execute(&format!(
            ...
            (0..column_count).map(|i| format!("col_{}", i)).collect::<Vec<_>>().join(", "),
            values.join(", ")
        )).await.unwrap();
        ...
        let messages = rivven.wait_for_messages(
            "cdc.postgres.public.chaos_wide",
            1,
            Duration::from_secs(15),
        ).await.unwrap();
        ...
        for i in 0..column_count {
            ...
            assert!(after.get(&col_name).is_some(), "Column {} should be captured", col_name);
        }
...
mod performance_tests {
    ...
    async fn test_baseline_throughput() {
        ...
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "perf_baseline").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
    ...
    async fn test_event_latency_baseline() {
        ...
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "perf_latency").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        ...
        for i in 0..samples {
            ...
            let _ = rivven.wait_for_messages(
...
mod edge_case_tests {
    ...
    async fn test_empty_string_values() {
        ...
        ctx.pg.execute(
            "CREATE TABLE edge_empty (id SERIAL PRIMARY KEY, name TEXT, description TEXT)"
        ).await.unwrap();
        
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "edge_empty").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        // Insert with empty strings
        ctx.pg.execute("INSERT INTO edge_empty (name, description) VALUES ('', '')").await.unwrap();
        ctx.pg.execute("INSERT INTO edge_empty (name, description) VALUES ('name', '')").await.unwrap();
        ctx.pg.execute("INSERT INTO edge_empty (name, description) VALUES ('', 'desc')").await.unwrap();
        
        sleep(Duration::from_millis(EVENT_PROPAGATION_DELAY_MS * 2)).await;
        
        let messages = rivven.wait_for_messages(
            "cdc.postgres.public.edge_empty",
            3,
            Duration::from_secs(15),
        ).await.unwrap();
        ...
        assert_eq!(first_after.get("description").unwrap().as_str().unwrap(), "");
    ...
    async fn test_numeric_edge_values() {
        ...
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "edge_numeric").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
    ...
    async fn test_reserved_keywords_as_identifiers() {
        ...
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "edge_keywords").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
    ...
    async fn test_composite_primary_key() {
        ...
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "edge_composite_pk").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        ...
        ctx.pg.execute("UPDATE edge_composite_pk SET name = 'Updated' WHERE tenant_id = 1 AND user_id = 1").await.unwrap();
    ...
    async fn test_table_without_primary_key() {
        ...
        ctx.pg.execute(
            "CREATE TABLE edge_no_pk (name TEXT, value INTEGER)"
        ).await.unwrap();
        // REPLICA IDENTITY FULL required for updates/deletes on tables without PK
        ctx.pg.execute("ALTER TABLE edge_no_pk REPLICA IDENTITY FULL").await.unwrap();
        
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "edge_no_pk").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        ctx.pg.execute("INSERT INTO edge_no_pk (name, value) VALUES ('test1', 100)").await.unwrap();
        ctx.pg.execute("INSERT INTO edge_no_pk (name, value) VALUES ('test2', 200)").await.unwrap();
        ctx.pg.execute("UPDATE edge_no_pk SET value = 150 WHERE name = 'test1'").await.unwrap();
        
        sleep(Duration::from_millis(EVENT_PROPAGATION_DELAY_MS * 2)).await;
        
        let messages = rivven.wait_for_messages(
            "cdc.postgres.public.edge_no_pk",
            3,
            Duration::from_secs(15),
        ).await.unwrap();
    ...
    async fn test_binary_data() {
        ...
        ctx.pg.execute(
            "CREATE TABLE edge_binary (id SERIAL PRIMARY KEY, data BYTEA)"
        ).await.unwrap();
        
        let mut rivven = RivvenTestContext::new().await.unwrap();
        rivven.register_cdc_topic("postgres", "public", "edge_binary").await.unwrap();
        rivven.start_cdc(&ctx.pg.cdc_connection_string(), &ctx.slot_name, &ctx.publication_name).await.unwrap();
        
        // Insert binary data using hex encoding
        ctx.pg.execute("INSERT INTO edge_binary (data) VALUES (E'\\\\x48656c6c6f')").await.unwrap(); // "Hello"
        ctx.pg.execute("INSERT INTO edge_binary (data) VALUES (E'\\\\x00010203')").await.unwrap(); // Binary with nulls
